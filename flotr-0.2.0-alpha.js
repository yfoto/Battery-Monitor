...
Flotr.Graph = Class.create({
            ...
            if (H.x.options.showLabels || H.x2.options.showLabels || H.y.options.showLabels || H.y2.options.showLabels) {
                ...
                if (D.options.showLabels) {
                    for (E = 0; E < D.ticks.length; ++E) {
                        ...
                        F.push('<div style="position:absolute;top:' + (this.plotOffset.top + this.plotHeight + J.grid.labelMargin) + "px;left:" + (this.plotOffset.left + this.tHoz(G.v, D) - B / 2) + "px;width:" + B + "px;text-align:center;" + (D.options.color ? ("color:" + D.options.color + ";") : "") + '" class="flotr-grid-label">' + G.label + "</div>")
                    }
                }
                D = H.x2;
    ...
    fireSelectEvent: function() {
        var A = this.axes,
            ...
            D = (F.first.y >= F.second.y) ? F.second.y : F.first.y;
